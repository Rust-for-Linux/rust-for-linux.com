...
The Rust versions currently supported should already be enough for kernel developers in recent distributions, such as:
  ...
  - Ubuntu 25.10 and 26.04 LTS. In addition, 24.04 LTS with versioned packages.
...
Since early 2026, our current policy is to follow Debian Stable's Rust versions as the minimum supported versions to use.

For instance, Debian 13 (Trixie)'s Rust version is 1.85.0 at the time of writing. Thus, after some months since Debian's release (to give some time for developers to upgade), it will become the Rust minimum (expected in Linux v7.1).
